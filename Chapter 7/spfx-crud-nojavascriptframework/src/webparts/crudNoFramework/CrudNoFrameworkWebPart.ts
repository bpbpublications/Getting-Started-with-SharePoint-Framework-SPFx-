...
import styles from './CrudNoFrameworkWebPart.module.scss';
import * as strings from 'CrudNoFrameworkWebPartStrings';

import { SPHttpClient, SPHttpClientResponse } from '@microsoft/sp-http';

export interface ICrudNoFrameworkWebPartProps {
  listName: string;
}
...
export default class CrudNoFrameworkWebPart extends BaseClientSideWebPart <ICrudNoFrameworkWebPartProps> {

  private listItemEntityTypeName: string = undefined;

  public render(): void {
    this.domElement.innerHTML = `
      <div class="${ styles.crudNoFramework }">
        <div class="${ styles.container }">
          <div class="${ styles.row }">
            <div class="${ styles.column }">
              <span class="${ styles.title }">CRUD Operations</span>
                <p class="${ styles.subTitle }">using No JavaScipt Framework.</p>                
  ...
  private setEventHandlers(): void {
    const webPart: CrudNoFrameworkWebPart = this;
  ...
  private getLatestItemId(): Promise<number> {
    return new Promise<number>((resolve: (itemId: number) => void, reject: (error: any) => void): void => {
      this.context.spHttpClient.get(`${this.context.pageContext.web.absoluteUrl}/_api/web/lists/getbytitle('${this.properties.listName}')/items?
      $orderby=Id desc&$top=1&$select=id`,
        SPHttpClient.configurations.v1,
        {
          headers: {
            'Accept': 'application/json;odata=nometadata',
